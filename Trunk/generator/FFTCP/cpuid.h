#ifndef __CPUID_FEATURE__
#define __CPUID_FEATURE__
...
void fetch_cpu_features(cpu_features_t *features)
{
	int info[4];
	cpuid(info, 0);
	int nIds = info[0];
	...
	if (nIds >= 0x00000001){
		...
		if (osUsesXSAVE_XRSTORE && cpuAVXSuport)
		{
			...
			features->AVX = (xcrFeatureMask & 0x6) == 0x6;
		}
	}
	if (nIds >= 0x00000007){
